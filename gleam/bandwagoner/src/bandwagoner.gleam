...
pub fn root_for_team(team: Team) -> Bool {
  case team {
    Team(coach: Coach(name: coach_name, ..), ..) if coach_name == "Gregg Popovich" ->
      True
    Team(coach: Coach(former_player: coach_is_former_player, ..), ..) if coach_is_former_player ->
      True
    Team(name: team_name, ..) if team_name == "Chicago Bulls" -> True
    Team(stats: Stats(wins: team_wins, ..), ..) if team_wins >= 60 -> True
    Team(stats: Stats(wins: team_wins, losses: team_losses), ..) if team_losses > team_wins ->
      True
    _ -> False
  }
}
